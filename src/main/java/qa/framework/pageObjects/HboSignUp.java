...
public class HboSignUp extends TestBase {
	...
	public HboSignUp selectOperator(String operator) throws Exception {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (isElementPresent(By
						.cssSelector("div.settings_description_white > #headerButtonLogin")))
					break;
			} catch (Exception e) {
			}
			Thread.sleep(1000);
		...
		driver.findElement(
				By.cssSelector("div.settings_description_white > #headerButtonLogin"))
				.click();
		...
		for (int second = 0;; second++) {
			if (second >= 60)
				fail("timeout");
			try {
				if (isElementPresent(By.name("SpecificData")))
					break;
			} catch (Exception e) {
			}
			Thread.sleep(1000);
		...
		for (int second = 0;; second++) {
			...
			try {
				if (isElementPresent(By
						.cssSelector("button.button_submit.next")))
					break;
			} catch (Exception e) {
			}
			Thread.sleep(1000);
